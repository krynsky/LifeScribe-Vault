...
describe("validatePack", () => {
  ...
  it("rejects a kit mapping that would print the password-manager master password", () => {
    const result = validatePack(
      mutatePack((pack) => {
        const section = (pack.sections as JsonSection[])[1];
        section.kitMapping.entries[0].fields.push("passwordManagerMasterPassword");
      }),
    );
    expect(result.ok).toBe(false);
    expect(result.errors).toContain(
      "Section password-manager: kit mapping may not include credential field passwordManagerMasterPassword — the Recovery Kit is a printable document.",
    );
  });

  it("rejects a kit mapping that would print a device PIN", () => {
    const result = validatePack(
      mutatePack((pack) => {
        const section = (pack.sections as JsonSection[])[2];
        section.kitMapping.entries[0].fields.push("devicePin");
      }),
    );
    expect(result.ok).toBe(false);
    expect(result.errors).toContain(
      "Section devices: kit mapping may not include credential field devicePin — the Recovery Kit is a printable document.",
    );
  });
