...
- Credential system keys (`passwordManagerMasterPassword`, `devicePin`) may never reach the Recovery Kit, which emits raw values with no redaction of its own. Enforced twice: `validatePack` rejects any pack whose `kitMapping` names one (authoring gate), and `buildRecoveryKit` drops them from every entry regardless of the pack (consumption gate). The second is load-bearing — a stored `customPack` reaches the Kit without ever passing `validatePack`.
